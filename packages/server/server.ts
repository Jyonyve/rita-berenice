...
import recapRoutes from './route/recap.routes.js';
...
import { APPNAME, DEFAULT_CHARACTER_AVATAR, DEFAULT_USER_AVATAR, MODULE_NAMES } from '@rita-berenice/shared/config';
...
const resolve = (p: string) => (isProduction ? path.resolve(process.cwd(), p) : path.resolve(__dirname, p));
...
async function createServer() {
  ...
  if (isProduction && !isObjectImageStorageConfigured()) {
    throw new Error('Production requires private object image storage; configure BUCKET_NAME and its S3 credentials');
  }
          ...
          functions: (originalImplementation) => ({
            ...
            createNewSession: async (input) => {
              ...
              return originalImplementation.createNewSession({
                ...input,
                accessTokenPayload: await buildRitaAccessTokenPayload(input.userId, input.accessTokenPayload),
              });
            },
          }),
        },
  ...
  app.use(compression());
  // Finalization sends the complete generated turn back to the server. Korean text can consume
  // roughly three UTF-8 bytes per character, so Express's 100 KB default was too close to the
  // normal model output envelope. Keep a deliberate transport guard without imposing a chat
  // character limit or truncating authoritative text.
  ...
  for (const runtimePrefix of runtimeImagePrefixes) {
    if (isObjectImageStorageConfigured()) {
      ...
      app.use(
        ...
        skipBundledDefaults(
          runtimePrefix,
          express.static(path.join(ensureLocalImageStorageRoot(), runtimePrefix.slice('/assets/'.length)), {
            fallthrough: false,
            maxAge: 0,
          }),
        ),
  ...
  app.use(`${BASE_API}/${MODULE_NAMES.RECAP}`, recapRoutes);
  ...
  app.get('/{*splat}', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      let render: (url: string, initialLang?: 'kor' | 'eng') => { html: string; emotionStyleTags: string };
