...
import { assertCharacterVisibleToUser, filterCharacterResponseByViewer } from '../store/characterStore.js';
import { asyncHandler, genRoutePattern, validateRequestData, validateServiceId } from '../util/routeHelpers.js';

import { EmotionKey, RUNTIME_CHARACTER_IMAGE_DIR, validEmotionKeys } from '@rita-berenice/shared/config';
import { characterUpload, deleteCharacterImage, processCharacterImagePair } from '../util/imageProcessingUtils.js';
...
import { buildCharacterGlossarySource, characterGlossaryJobService } from '../service/characterGlossaryJobService.js';
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (!Number.isInteger(parsedEmotionKey) || !validEmotionKeys.has(parsedEmotionKey as EmotionKey)) {
