...
interface BackgroundJobQueueOptions<TInput, TResult> {
  ...
  onChangeError?: (error: unknown, snapshot: BackgroundJobSnapshot<TResult>, input: TInput) => Promise<void> | void;
    ...
    while (job.attempts < job.maxAttempts) {
      ...
      try {
        ...
        if (job.attempts < job.maxAttempts) {
          job.status = 'retrying';
          await this.notifyChange(job);
          await new Promise((resolve) => setTimeout(resolve, this.retryDelayMs * 2 ** (job.attempts - 1)));
        }
      }
    }
    job.status = 'failed';
