...
import { ChatEntry, DefaultAiRole, HistoryInfo, LoreInfo, ChatTurn } from '@rita-berenice/shared/domain';
...
export const rankBySemanticScore = (
  ...
  const rawDistances = hits.map((h) => (typeof h.distance === 'number' ? h.distance : Number.POSITIVE_INFINITY));
