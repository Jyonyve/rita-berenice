...
export const buildCharacterPortraitUrls = (
	...
	for (const fileName of [...fileNames].sort()) {
		...
		if (
			existingUrl &&
			assetUrlEndsWithExtension(existingUrl, preferredExtension) &&
			extension !== preferredExtension
		) {
			continue;
		}

		portraitUrls[emotionKey] = appendImageAssetVersion(
			toAssetUrl(characterId, fileName),
			versions[fileName]
		);
	}
...
export const buildCharacterAvatarUrls = (
	...
	for (const fileName of [...fileNames].sort()) {
		...
		avatarUrls[emotionKey] = appendImageAssetVersion(
			toAssetUrl(characterId, fileName),
			versions[fileName]
		);
	}
...
export const getProfileImageUrls = async (
	profileId: string
): Promise<{ portraitUrl?: string; avatarUrl?: string }> => {
...
export const buildHistoryImageUrl = (
	...
	return matchingFiles[0]
		? appendImageAssetVersion(
				toAssetUrl(characterId, 'lore', matchingFiles[0]),
				versions[matchingFiles[0]]
			)
		: undefined;
};
