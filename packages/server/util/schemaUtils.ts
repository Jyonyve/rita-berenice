...
const chatTurnInputMessageSchema = z
  .object({
    ...
    generationStatus: z.enum(['complete', 'length_limited']).optional(),
...
export const ContinueTempResponseBodySchema = z
  .object({
    sessionId: z.string().min(1),
    sequence: z.number().int().nonnegative(),
    setNo: z.number().int().nonnegative(),
  })
  .strict();
...
export const ChatTurnCdoSchema = z
  ...
  .superRefine((turn, context) => {
    ...
    for (const message of messages) {
      ...
      if (message.value.messageType !== message.expectedType || message.value.role !== message.expectedRole) {
        context.addIssue({
          code: 'custom',
          path: [message.key],
...
export const UpdateChatTurnBodySchema = z
  .object({
    request: chatTurnInputMessageSchema.optional(),
    response: chatTurnInputMessageSchema.optional(),
  })
  .strict()
  .refine((body) => body.request !== undefined || body.response !== undefined, {
    message: 'At least one chat message must be provided.',
  })
  .superRefine((body, context) => {
    if (body.request && (body.request.messageType !== 'request' || body.request.role !== 'user')) {
      context.addIssue({
        code: 'custom',
        path: ['request'],
        message: 'The request message must have request/user identity.',
      });
    }
    if (body.response && (body.response.messageType !== 'response' || body.response.role !== 'assistant')) {
      context.addIssue({
        code: 'custom',
        path: ['response'],
        message: 'The response message must have response/assistant identity.',
      });
    }
  });
 ...
 */
export const createPersonaResponseSchema = (charName: string, userName: string, langCode: 'kor' | 'eng' = 'kor') =>
  z
    .object({
      groundingDecision: z
...
export const createPersonaContinuationSchema = (charName: string, langCode: 'kor' | 'eng' = 'kor') =>
  z.object({
    response: z
      .string()
      .min(1)
      .describe(
        langCode === 'kor'
          ? `${charName}의 미완성 응답 바로 뒤에 붙일 새 접미부만 작성한다. 기존 내용을 반복하거나 다시 쓰지 않는다.`
          : `Only the new suffix to append directly to ${charName}'s incomplete response. Never repeat or rewrite existing text.`,
      ),
    emotion: z.enum(emotionList).describe('Single final emotion word from the provided list.'),
  });
...
export const createChatTurnMetadataSchema = (charEng: string, userEng: string) => {
  ...
  return z
    .object({
      ...
      topicList: z
        .array(z.string())
        .describe("An array of broader themes in snake_case, e.g., ['character_background', 'trust_issues']"),
      ...
      userEmotion: z.object({
        ...
        intensity: z.number().min(0.0).max(1.0).describe('The intensity of the primary emotion, from 0.0 to 1.0.'),
        nuanceList: z
          .array(z.string())
      ...
      characterEmotion: z.object({
        ...
        intensity: z.number().min(0.0).max(1.0).describe('The intensity of the primary emotion, from 0.0 to 1.0.'),
        nuanceList: z.array(z.string()).describe("An array of specific emotion words, e.g., ['defensive', 'sadness']"),
      }),
      ...
      dialogueAct: z.string().describe("The conversational act, e.g., 'question', 'answer', 'revelation', 'evasion'"),
      ...
      flagList: z
        .array(z.string())
        .describe("An array of flags for significant events, e.g., ['new_lore_revealed', 'major_plot_point']"),
...
export const createLoreMetadataSchema = (availableCharacterIds: string[]) => {
  return z
    .object({
      ...
      source: z.string().describe("Where this lore originates from, e.g., 'Ancient Texts', 'Character Dialogue'."),
      ...
      entityList: z.array(z.string()).describe('An array of named entities mentioned (people, places, items).'),
    })
    .describe(
      'Lore metadata schema: Structured world-building information including title, categorization, and searchable metadata for RAG system integration.',
    );
};
...
export const createHistoryMetadataSchema = (
  ...
  return z
    .object({
      // Core Identification
      generatedTitle: z.string().describe('A concise, descriptive title in English for the event. Maximum 3 words.'),
      summary: z.string().describe('A one-paragraph summary of the event, capturing the key actions and outcomes.'),
      category: z
        .enum([
          'Origin Story',
          'Major Life Event',
      ...
      period: z.object({
        label: z.string().describe("A descriptive label for the life period, e.g., 'Childhood', 'Reign', 'Exile'."),
        confidence: z.number().min(0.0).max(1.0).describe('Confidence in the period label from 0.0 to 1.0.'),
      }),
      eventDate: z.object({
      ...
      relatedEventList: z
        .array(
          z.object({
            type: z.enum(['PRECEDES', 'SUCCEEDS', 'CONCURRENT_WITH', 'OVERLAPS_WITH', 'CAUSED_BY', 'RESULTS_IN']),
            relatedEventTitle: z
              .string()
              .describe(
      ...
      entityList: z.array(z.string()).describe('An array of named entities mentioned (people, places, events).'),
    })
    .describe(
 ...
 */
// NOT WIRED: legacy output contract for `buildFactualRecapPrompt`. The production four-turn
// flow uses `generatedFactualRecapSchema`; keep this richer draft for future enrichment work.
export const createFactualRecapSchema = (
  ...
  z
    .object({
      ...
      topics: z
        .array(z.string())
        .describe(`An array of 3-7 key factual themes, selected from: [${convertArrayToString(availableTopics)}]`),
      entities: z
        .array(z.string())
        .describe(`An array of important entities, selected from: [${convertArrayToString(availableEntities)}]`),
      flags: z
        .array(z.string())
        .describe(
...
export const createNerSchema = () =>
  z
    .object({
      properNouns: z.array(z.string()).describe('An array of unique proper nouns extracted from the text.'),
    })
    .describe(
  ...
  .object({
    entities: z
      .object({
        ...
        items: z.array(z.string()).optional().describe('A list of important items or objects mentioned.'),
      })
      .optional()
      .describe('Key entities identified in the query.'),

    keywords: z.array(z.string()).optional().describe('A list of single-word keywords from the query.'),
    ...
    emotion: z
      .string()
      .optional()
      .describe("The dominant emotion conveyed by the user's query (e.g., 'angry', 'curious', 'happy')."),
    criticalTerm: z
      .string()
      .optional()
      .describe(
...
export const FilterCriteriaSchema = z
  .object({
    ...
    period: z.string().optional().describe("The relevant time period, in English (e.g., 'Childhood', 'Reign')."),
  })
  .describe(
