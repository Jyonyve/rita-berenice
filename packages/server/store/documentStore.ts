...
const normalizeDocumentInfo = (document: DocumentInfo): DocumentInfo => documentInfoSchema.parse(document);
...
export const documentToEmbeddingContent = (document: DocumentInfo): string =>
  [
    ...
    document.timelineOrder !== undefined ? `In-world timeline order: ${document.timelineOrder}` : undefined,
...
export const applyDocumentApproval = (document: DocumentInfo, updatedAt: string): DocumentInfo => {
  ...
  if (!document.body.trim()) throw new ApiError(400, 'A document must have content before approval.');
  return {
    ...document,
    status: 'approved',
    revision: document.revision + 1,
...
export const applyDocumentRetrievalPreference = (
  ...
  if (document.retrievalEnabled === enabled) return document;
  return {
    ...document,
...
export const documentStore = {
  createDraft: async (input: CreateDocumentDraftInput): Promise<DocumentInfo> => {
    ...
    await getDatabase().insert(documents).values({
  ...
  updateDraft: async (documentId: string, userId: string, input: DocumentDraftUpdate): Promise<DocumentInfo> => {
  ...
  approve: async (documentId: string, userId: string): Promise<DocumentInfo> => {
    ...
    const rows = await getDatabase()
      ...
      .where(and(eq(documents.documentId, documentId), eq(documents.userId, userId), eq(documents.status, 'draft')))
      .returning({ data: documents.data });
  ...
  setRetrievalPreference: async (documentId: string, userId: string, enabled: boolean): Promise<DocumentInfo> => {
    const current = await getOwnedDocument(documentId, userId);
    const now = new Date().toISOString();
    ...
    const rows = await getDatabase()
      ...
      .where(and(eq(documents.documentId, documentId), eq(documents.userId, userId), eq(documents.status, 'approved')))
      .returning({ data: documents.data });
