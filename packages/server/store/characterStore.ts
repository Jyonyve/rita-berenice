...
export const filterCharacterResponseByViewer = (
  ...
  const visibleInfos = response.characterInfos.filter(
    (character) => character.userId === viewerUserId || character.visibility !== CHARACTER_VISIBILITY.PRIVATE,
  );
  ...
  return {
    ...
    characterPortraits: Object.fromEntries(
      Object.entries(response.characterPortraits).filter(([characterId]) => visibleIds.has(characterId)),
    ),
...
export const assertCharacterVisibleToUser = (response: CharacterResponse, viewerUserId: string): CharacterResponse => {
...
export const characterStore = {
  ...
  storeCharacter: async (character: CharacterCdo | CharacterInfo): Promise<{ characterId: string }> => {
    const now = new Date().toISOString();
    const baseCharacter = isCharacterInfo(character) ? character : createBasicCharacterInfo(character);
