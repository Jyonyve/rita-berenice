...
const toResponse = (items: (CharacterTermInfo | SessionTermInfo)[]): TermResponse => {
  ...
  return {
    terms: promptTerms,
    term: promptTerms[0],
    characterTermInfos: items.filter((item): item is CharacterTermInfo => item.type === METADATA_TYPES.CHARACTER),
    sessionTermInfos: items.filter((item): item is SessionTermInfo => item.type === METADATA_TYPES.SESSION),
  };
};
...
const upsert = async (item: CharacterTermInfo | SessionTermInfo): Promise<void> => {
  ...
  await getDatabase()
    ...
    .values({
      termId: item.termId,
      termType: item.type,
      scopeId: item.type === METADATA_TYPES.SESSION ? (item as SessionTermInfo).sessionId : item.characterId,
...
export const termStore = {
  ...
  storeCharacterTerm: async (input: CharacterTermCdo | CharacterTermInfo): Promise<{ termId: string }> => {
    const now = new Date().toISOString();
  ...
  storeCharacterTerms: async (items: (CharacterTermCdo | CharacterTermInfo)[]): Promise<{ termIds: string[] }> => {
    const results = [];
  ...
  storeSessionTerms: async (items: (SessionTermCdo | SessionTermInfo)[]): Promise<{ termIds: string[] }> => {
    const results = [];
  ...
  ensureAndGetCharacterTerms: async (
    ...
    const result = new Map<string, string>();
    for (const koreanTerm of new Set(koreanTermsToEnsure.map((term) => term.trim()).filter(Boolean))) {
