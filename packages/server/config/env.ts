...
const credentialEnvSchema = z.object({
  SECRET_ENCRYPTION_KEY: z.string().min(1, 'SECRET_ENCRYPTION_KEY is required for credential storage'),
});
...
const imageStorageEnvSchema = z
  ...
  .superRefine((value, context) => {
    const requiredKeys = ['BUCKET_NAME', 'AWS_ENDPOINT_URL_S3', 'AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY'] as const;
