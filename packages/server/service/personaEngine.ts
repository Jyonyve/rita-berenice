...
import { createPersonaContinuationSchema, createPersonaResponseSchema } from '../util/schemaUtils.js';
...
import { extractPartialJsonString, PartialJsonStringDecoder } from '../util/partialJsonUtils.js';
...
export type PersonaGroundingDecision = 'not_applicable' | 'supported' | 'contradicted' | 'uncertain';
...
export const personaEngine = {
  ...
  async generateResponse(
    ...
    try {
      let response = options?.onDelta
        ...
        : await llmService.invokeStructuredLlm(messages, aiModelInfo, profileInfo.userId, personaSchema, options);
      ...
      if (parsingError instanceof OutputLengthLimitError) {
        const partialResponse = extractPartialJsonString(parsingError.rawOutput, 'response');
        if (partialResponse) {
          flowLogger.warn('personaEngine', 'outputLengthLimit.partialPreserved', {
            ...logContext,
            rawOutputLength: parsingError.rawOutput.length,
            responseLength: partialResponse.length,
          });
          return { response: partialResponse, emotion: 'neutral', generationStatus: 'length_limited' };
        }
      }
      ...
      try {
        ...
        flowLogger.error('personaEngine', 'structuredOutput.repairFailed', {
          ...
          correctionError: correctionError instanceof Error ? correctionError.message : 'Unknown correction error',
        });
  ...
  async continueResponse(
    ...
    const messages: ChatCompletionMessageParam[] = [
      ...
      buildChatCompletion('assistant', partialResponse, characterInfo.showName),
      buildChatCompletion('user', buildPersonaContinuationPrompt(characterInfo.showName, recalledMemories.langCode)),
    ];
    const continuationSchema = createPersonaContinuationSchema(characterInfo.showName, recalledMemories.langCode);
    const logContext = {
      userId: profileInfo.userId,
      characterId: characterInfo.characterId,
      sessionId: profileInfo.sessionId,
    ...
    try {
      ...
      const result = options?.onDelta
        ? await llmService.streamStructuredLlm(
            messages,
            aiModelInfo,
            profileInfo.userId,
            (rawDelta) => {
        ...
        : await llmService.invokeStructuredLlm(messages, aiModelInfo, profileInfo.userId, continuationSchema, options);
      flowLogger.info('personaEngine', 'continueResponse.complete', {
        ...logContext,
        suffixLength: result.response.length,
      });
      return { response: result.response, emotion: result.emotion, generationStatus: 'complete' };
    } catch (error: unknown) {
      if (error instanceof OutputLengthLimitError) {
        const partialSuffix = extractPartialJsonString(error.rawOutput, 'response');
        if (partialSuffix) {
          flowLogger.warn('personaEngine', 'continueResponse.partialPreserved', {
            ...logContext,
            suffixLength: partialSuffix.length,
          });
          return { response: partialSuffix, emotion: 'neutral', generationStatus: 'length_limited' };
        }
      }
      throw error;
    }
...
const reviseContradictedResponse = async (
  ...
  const revisionMessages: ChatCompletionMessageParam[] = [
    ...messages,
    buildChatCompletion('system', buildContradictedResponseRevisionPrompt(characterName, userName, langCode)),
    buildChatCompletion('user', `Rejected draft:\n${rejectedResponse}`),
  ];

  if (!options?.onDelta) {
    return llmService.invokeStructuredLlm(revisionMessages, aiModelInfo, userId, personaSchema, options);
  }
...
export const buildPersonaMessages = (
  ...
  const { langCode, shortTermHistory } = recalledMemories;
  const staticSystemPrompt = buildStaticSystemPrompt(characterInfo, profileInfo, langCode, adultContentEnabled);
  const longTermMemoryContent = buildLongTermMemoryPrompt(recalledMemories, langCode);
  const responseContract = buildPersonaResponseContract(characterInfo.showName, profileInfo.showName, langCode);
...
const buildShortTermHistoryMessages = (shortTermHistory: ChatTurn[]): ChatCompletionMessageParam[] => {
  ...
  return shortTermHistory.flatMap((turn) => {
    ...
    if (turn.request?.entries) {
      turnMessages.push(
        buildChatCompletion('user', parseEntriesToConversation(turn.request.entries), turn.request.showName),
      );
    }
    ...
    if (turn.response?.entries) {
      turnMessages.push(
        buildChatCompletion('assistant', parseEntriesToConversation(turn.response.entries), turn.response.showName),
      );
    }
