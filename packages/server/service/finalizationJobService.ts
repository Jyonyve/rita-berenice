import { EnqueueFinalizationResponse, FinalizationJobSnapshot, FinalizationJobStatus } from '@rita-berenice/shared/api';
import { ApiKeyError, ApiKeyType, ChatTurn, ChatTurnCdo, DisplayTurn } from '@rita-berenice/shared/domain';
...
import { chatStore, StoreChatTurnOptions } from '../store/chatStore.js';
import { BackgroundJobErrorMeta, BackgroundJobQueue, BackgroundJobSnapshot } from '../util/backgroundJobQueue.js';
...
export interface FinalizationJobDeps {
  storeChatTurn: (turn: ChatTurn, options?: StoreChatTurnOptions) => Promise<unknown>;
  getStoredTurn: (chatTurnId: string) => Promise<ChatTurn | undefined>;
  enrichChatTurn: (basicTurn: ChatTurn) => Promise<ChatTurn>;
  enqueueRecapForTurn: (turn: ChatTurn) => Promise<void>;
  persistJob: (job: FinalizationJobSnapshot, input: ChatTurnCdo) => Promise<void>;
  readPersistedJob: (jobId: string) => Promise<{ job: FinalizationJobSnapshot; input?: ChatTurnCdo } | undefined>;
  listResumableJobs: (limit: number) => Promise<ResumableFinalizationJob[]>;
  markResumed: (jobIds: string[]) => Promise<void>;
}

const persistFinalizationJob = async (job: FinalizationJobSnapshot, input: ChatTurnCdo): Promise<void> => {
...
export const databaseFinalizationJobDeps: FinalizationJobDeps = {
  storeChatTurn: (turn, options) => chatStore.storeChatTurn(turn, options),
  ...
  enqueueRecapForTurn: (turn) => enqueueRecapGenerationAfterFinalization(turn),
...
export const createFinalizationJobService = (deps: FinalizationJobDeps = databaseFinalizationJobDeps) => {
  ...
  const queue = new BackgroundJobQueue<ChatTurnCdo, ChatTurn>({
    worker: async (chatTurnCdo) => {
      ...
      if (!enrichedTurn) {
        ...
        // Persist the recoverable raw turn without embedding it. The enriched re-store below
        // is the single point that enqueues the final authoritative turn for embedding.
        await deps.storeChatTurn(basicTurn, { enqueueEmbedding: false });
      ...
      await deps.enqueueRecapForTurn(enrichedTurn);
  ...
  return {
    async enqueue(chatTurnCdo: ChatTurnCdo): Promise<EnqueueFinalizationResponse> {
      ...
      if (persisted && persisted.job.status !== 'failed') {
        ...
        if (persisted.job.status === 'completed' && persisted.job.result) {
          await deps.enqueueRecapForTurn(persisted.job.result);
        }
      ...
      if (finalizedTurn) await deps.enqueueRecapForTurn(finalizedTurn);
