...
export const buildDocumentSourceRefs = (memories: MemoryResponse): DocumentSourceRefs => ({
  chatTurnIds: unique([...memories.shortTermHistory, ...memories.longTermHistory].map((turn) => turn.chatTurnId)),
...
export const documentGenerationService = {
  generateDraft: async (input: GeneratedDocumentDraftCreate, userId: string, session: SessionInfo) => {
    const [characterResponse, profileResponse, recentChatResponse, aiModelInfo] = await Promise.all([
      characterStore.getCharacter(session.characterId),
      profileStore.getProfileBySessionId(session.sessionId),
      chatStore.getRecentChatTurns(session.sessionId),
      modelCatalogService.resolveAiModelInfo(input.modelName),
    ]);
    const recentTurns = recentChatResponse.chatTurns;
...
Use the user's language unless the requested artifact clearly requires another language.`,
    ...
    const generated = await llmService.invokeStructuredLlm(messages, aiModelInfo, userId, generatedDocumentSchema);
  ...
  rewriteDraft: async (documentId: string, input: DocumentDraftRewrite, userId: string, session: SessionInfo) => {
    const [current, characterResponse, profileResponse, recentChatResponse, aiModelInfo] = await Promise.all([
