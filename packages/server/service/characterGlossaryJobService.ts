...
type CharacterGlossaryWorker = (input: CharacterGlossaryJobInput) => Promise<CharacterGlossaryJobResult>;

export const buildCharacterGlossarySource = (
  character: Pick<CharacterInfo, 'name' | 'showName' | 'title' | 'worldIntroduction' | 'description' | 'instruction'>,
): string =>
  [
    character.name,
    character.showName,
...
const scanCharacterGlossary: CharacterGlossaryWorker = async (input) => {
  ...
  const extractedTerms = await llmService.extractGlossaryTerms(input.sourceText, input.userId, utilityModelInfo);
  const resolvedTerms = await termStore.storeMissingCharacterTermMappings(input.characterId, extractedTerms);
  return {
...
export const createCharacterGlossaryJobService = (worker: CharacterGlossaryWorker = scanCharacterGlossary) => {
