...
export const ragQueryService = {
  ...
  async transformQuery(
    ...
    try {
      ...
      const finalTranslation = translatedUserInput.status === 'fulfilled' ? translatedUserInput.value : userInput; // Fallback to original

      // Handle extraction result
      const finalExtraction = extractedData.status === 'fulfilled' ? extractedData.value : ({} as FilterCriteria); // Fallback to empty
