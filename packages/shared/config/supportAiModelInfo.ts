 ...
 *
 * These calls used to run unconditionally on `DEFAULT_EXTRACTION_MODEL` (direct/openai). That is
 * a bug, not a cost choice: an account that chats on Google or OpenRouter
 ...
 * points at models, it never introduces one. Direct OpenAI deliberately uses the balanced Luna
 * tier. Other providers use a compatible utility tier because key compatibility remains mandatory.
 */
 ...
 *
 * `direct/openai` is first so accounts with an OpenAI key use the balanced Luna tier. The rest run
 * direct before OpenRouter (no routing markup) and lower-cost tiers before expensive ones.
 */
