...
export const removeCharacterFromLore = (currentCharacterIds: string, characterIdToRemove: string): string => {
 ...
 */
export const metadataToChatTurn = (metadata: ChatTurnMetadata, indexRecords: ChatIndexMetadata[] = []): ChatTurn => {
  ...
  const getValuesFromIndex = (contentType: ChatIndexContentType): string[] => {
    return indexRecords.filter((record) => record.contentType === contentType).map((record) => record.value);
  };
  ...
  return {
    ...
    historyReferenceList: metadata.historyReferenceList ? JSON.parse(metadata.historyReferenceList) : [],
...
export const metadataToHistory = (
  ...
  const getValuesFromIndex = (contentType: HistoryIndexContentType): string[] => {
    return indexRecords
      .filter((record) => record.contentType === contentType && record.historyId === metadata.historyId)
      .map((record) => record.value);
  };
...
export const addLoreReference = (existingReferences: Reference[], newReference: Reference): Reference[] => {
...
export const removeLoreReference = (existingReferences: Reference[], referenceIdToRemove: string): Reference[] => {
  return existingReferences.filter((ref) => ref.id !== referenceIdToRemove);
};

export const addHistoryReference = (existingReferences: Reference[], newReference: Reference): Reference[] => {
  // Same logic as lore references
  return addLoreReference(existingReferences, newReference);
};

export const removeHistoryReference = (existingReferences: Reference[], referenceIdToRemove: string): Reference[] => {
