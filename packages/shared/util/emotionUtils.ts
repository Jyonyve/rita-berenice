import { numberToEmotionWordsMap, EmotionKey, EmotionValue, DEFAULT_EMOTION } from '../config/index.js';
 ...
 */
export const getSimilarEmotions = (emotion: string, excludeSelf: boolean = true): readonly string[] => {
 ...
 */
export const EMOTION_COMPATIBILITY_MATRIX: Record<EmotionKey, Partial<Record<EmotionKey, number>>> = {
...
export const calculateEmotionCompatibility = (emotion1: string, emotion2: string): number => {
  ...
  const compatibility =
    EMOTION_COMPATIBILITY_MATRIX[category1]?.[category2] || EMOTION_COMPATIBILITY_MATRIX[category2]?.[category1] || 0;
