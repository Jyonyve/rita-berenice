...
const getDerivedKey = async (secret: string, salt: ArrayBuffer): Promise<CryptoKey> => {
  ...
  const keyMaterial = await crypto.importKey('raw', new TextEncoder().encode(secret), { name: 'PBKDF2' }, false, [
    'deriveKey',
  ]);
...
export const encryptValue = async (plaintext: string, secret: string): Promise<string> => {
  ...
  const encryptedB64 = btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(encryptedContent))));
