import { ChatIndexContentType, ChatMessageType, LoreIndexContentType, RecapIndexContentType } from '../domain/index.js';
...
export const buildMessageId = (sessionId: string, sequence: number, messageType: ChatMessageType): string => {
...
export const buildChatTurnIndexId = (chatTurnId: string, contentType: ChatIndexContentType): string => {
...
export const buildHistoryIndexId = (historyId: string, contentType: LoreIndexContentType): string => {
  return `${historyId}_${sanitizeIdInput(contentType)}_${_genNanoId(4)}`;
};

export const buildRecapIndexId = (chatTurnId: string, contentType: RecapIndexContentType): string => {
  return `${chatTurnId}_${sanitizeIdInput(contentType)}_${_genNanoId(4)}`;
};
...
export const buildRelationshipRecapId = (sessionId: string, turnStart: number, turnEnd: number): string => {
