...
import { AccountMenuControl, type AccountMenuControlProps } from './component/AccountMenuControl.js';
...
type SessionHeaderProps = {
  ...
  onProfile: () => void;
  onSession: () => void;
  onSessionTitleSave: (title: string) => Promise<void>;
  onSessionLore: () => void;
  onDocuments: () => void;
  onImage: () => void;
  hidden?: boolean;
      ...
      <Toolbar sx={{ minHeight: { xs: 52, sm: 56 }, gap: 1 }}>
        ...
        >
          ...
          >
            ...
            {info.sessionId && info.sessionTitle !== undefined && (
              <InlineEditableField
                ...
                inputAriaLabel={getLangText(LANG_KEYS.SESSION_TITLE)}
          ...
          <RomanticTitle
            ...
            colorVariant="silver"
            component="div"
            onClick={onSessionLore}
            role="button"
            tabIndex={0}
            onKeyDown={(event) => {
              if (event.key === 'Enter' || event.key === ' ') onSessionLore();
            }}
            sx={{ display: { xs: 'none', md: 'block' }, px: 1, whiteSpace: 'nowrap', cursor: 'pointer' }}
          >
            {getLangText(LANG_KEYS.SESSION_LORE)}
          </RomanticTitle>
          <RomanticTitle
            variant="subtitle1"
            colorVariant="silver"
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
          <Tooltip title={getLangText(LANG_KEYS.SESSION_LORE)}>
            <HeaderIconButton
              size="small"
              onClick={onSessionLore}
