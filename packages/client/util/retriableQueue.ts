...
export const runRetriableQueue = async <T>(
	...
	const results: Array<{ succeeded: boolean; error?: unknown } | undefined> = new Array(
		items.length
	);
	let nextIndex = 0;
	let completed = 0;
	...
	return {
		...
		failed: items.flatMap((item, index) =>
			results[index]?.succeeded ? [] : [{ item, error: results[index]?.error }]
		),
	};
};
