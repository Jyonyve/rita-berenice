...
export const useSessionApi = () => {
   ...
   */
  const updateSessionOnNewMessage = useMutation<void, Error, { sessionId: string; latestCharMessage: string }>({
    mutationFn: async (variables) => {
  ...
  const updateSessionTitle = useMutation<void, Error, { sessionId: string; title: string }>({
    ...
    onSuccess: async (_, variables) => {
      // Match the actual getSession key and refresh every list variant that includes this title.
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: ['sessions', 'detail', 'getSession', variables.sessionId],
        }),
