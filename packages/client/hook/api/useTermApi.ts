...
export const useTermApi = () => {
  ...
  const storeSessionTerm = useMutation<{ termId: string }, Error, SessionTermCdo | SessionTermInfo>({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['sessionTerms', 'detail', 'getTermByKorean', variables.sessionId, variables.koreanTerm],
      });
  ...
  const storeSessionTerms = useMutation<{ termIds: string[] }, Error, { terms: (SessionTermCdo | SessionTermInfo)[] }>({
  ...
  const storeCharacterTerm = useMutation<{ termId: string }, Error, CharacterTermCdo | CharacterTermInfo>({
    mutationFn: async (termInfo) => {
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['characterTerms', 'detail', 'getTermByKorean', variables.characterId, variables.koreanTerm],
      });
