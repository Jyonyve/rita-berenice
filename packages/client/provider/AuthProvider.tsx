...
import React, { createContext, useState, useContext, FC, ReactNode, useEffect, useRef, useCallback } from 'react';
...
export const AuthProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchAndSetUserProfile = useCallback(async () => {
    ...
    try {
      ...
      const { data: freshUserData, error: refetchError } = await refetchUserQuery();
      ...
      if (freshUserData?.userInfo) {
        ...
        console.error('❌ [Auth] Error fetching user profile:', refetchError);
        setUserProfile(undefined);
        setNeedsProfileSetup(false);
      } else {
        // A successful empty response represents an authenticated user without a Rita profile.
        setUserProfile(undefined);
  ...
  }, [userQueryData, isLoggedIn, isUserQueryLoading, isUserQueryError]);
  ...
  const value: AuthContextType = {
    ...
    userProfile,
    profileLoadError:
      isLoggedIn && isUserQueryError && !userQueryData?.userInfo ? (userQueryError ?? undefined) : undefined,
    isProfileRefreshing: isLoggedIn && isUserQueryFetching,
    needsProfileSetup,
    createUserProfile,
    refetchProfile,
    isAdmin,
