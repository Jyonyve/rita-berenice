...
import { Typography, CircularProgress, Box, Dialog, DialogContent, IconButton, useTheme } from '@mui/material';
import CloseIcon from '@mui/icons-material/Close';
import { useChatApi, useCharacterApi, useProfileApi, useTempChatApi, useSessionApi } from '../../hook/api/index.js';
...
export function ChatPageLoader() {
  ...
  const { setHeaderInfo, setSessionHeaderHidden, headerInfo } = useOutletContext<HeaderContextType>();
  ...
  // ------------ Fetching Data ------------
  const characterId = useMemo(() => parseSessionId(safeSessionId)?.characterId || '', [safeSessionId]);
  ...
  const { isLoading: isLoadingTurns, isError: isTurnsError } = useChatApi().getAllDisplayTurns(safeSessionId);
  ...
  const imageUrl = useMemo(() => {
    if (characterRes?.characterInfo) {
      return (
        getImageForEmotion(characterRes.characterPortraits[characterRes.characterInfo.characterId], currentEmotion) ||
        ''
      );
    }
    return '';
  ...
  }, [characterRes, profileRes, sessionRes, setHeaderInfo, shouldUseMobileLayout, imageUrl, safeSessionId]);
