...
export const FixedTurnDisplay: FC<FixedTurnDisplayProps> = memo(
  ({
    ...
    const avatarUrl = getConversationAvatar(characterAvatarUrls, characterPortraitUrls, turn.response.emotion);
    ...
    const originalBotText = parseEntriesToText(turn.response.entries);
    ...
    return (
      ...
      >
        {isEditing ? (
          <GlassBox gap={2}>
            ...
            <TextField
              ...
              value={botEditInput}
              onChange={(e) => {
                if (isResponseEditChangeAllowed(botEditInput, e.target.value)) setBotEditInput(e.target.value);
              }}
              disabled={isBusy}
              slotProps={{
                htmlInput: {
        ...
        {!isEditing && turn.response.generationStatus === 'length_limited' ? (
          ...
          {isEditing ? (
            <>
              ...
              <IconButton
                ...
                disabled={
                  isBusy || !userEditInput.trim() || !botEditInput.trim() || isUserTextOverflow || isBotEditInvalid
                }
