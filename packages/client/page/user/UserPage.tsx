...
import { genderToLangKey, getGenderSelectLabel, getLangAlertText, getLangText } from '../../util/translateUtils.js';
...
import { CharacterInfo, SessionInfo, ApiKeyType, UploadedImage, UserUdo, UserInfo } from '@rita-berenice/shared/domain';
...
export const UserPage: FC<{
  ...
  const { shouldUseMobileLayout, isSmallScreen, isTabletPortrait, hasEnoughSpaceForDesktop, isWideTablet } =
    useResponsive();
  ...
  const isApiKeySetupRequested = isMine && searchParams.get(API_KEY_SETUP_PARAM) === API_KEY_SETUP_VALUE;
  ...
  const onSubmit = async (formData: UserUdo) => {
    try {
      ...
      const message = getClientErrorMessage(error, 'An error occurred while updating the profile.');
                    ...
                    {isEditing ? (
                      <Stack direction="row" spacing={1}>
                        <Tooltip title={getLangText(LANG_KEYS.SAVE)}>
                          <span>
                            <IconButton
                              type="submit"
                              color="secondary"
                              aria-label={getLangText(LANG_KEYS.SAVE)}
                        ...
                        <Tooltip title={getLangText(LANG_KEYS.CANCEL)}>
                          <IconButton
                            ...
                            color="secondary"
                            aria-label={getLangText(LANG_KEYS.CANCEL)}
                            disabled={isSubmitting || isUploading}
                          >
                            <CancelIcon />
                          </IconButton>
                      ...
                      <Tooltip title={getLangText(LANG_KEYS.EDIT_USER_INFO)}>
                        <IconButton
                          ...
                          aria-label={getLangText(LANG_KEYS.EDIT_USER_INFO)}
                        >
                          <EditIcon />
                        </IconButton>
                      </Tooltip>
                      ...
                      <Box display="flex" justifyContent="space-between" alignItems="center">
                        ...
                        <Box display="flex" alignItems="center" gap={1}>
                          <IconButton
                            ...
                            aria-label={apiKeysExpanded ? 'Collapse API keys' : 'Expand API keys'}
