...
import { ImageCropModal, RomanticTitle, SolidMetallicButton, PortraitWithChip } from '../../layout/component/index.js';
import {
  EmotionKey,
  EmotionValue,
  DEFAULT_EMOTION,
...
import { LoreManagerDialog } from '../lore/LoreManagerDialog.js';
...
// 🎨 Helper Functions
const getVisibleImages = (images: UploadedCharacterImage[]) => images.filter((img) => !img?.toDelete);
...
export const CharacterForm: FC<Props> = ({
  ...
  const { storeCharacter, uploadCharacterImage, createCharacterFolder, deleteCharacterImage, refreshCharacterImages } =
    useCharacterApi();
  const { storeLore } = useLoreApi();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const swiperRef = useRef<SwiperClass | null>(null);
  const pendingCreateIdentityRef = useRef<Pick<CharacterInfo, 'characterId' | 'variant' | 'createdAt'> | undefined>(
    undefined,
  );
    ...
    try {
      ...
      if (mode === 'edit' && characterInfo?.characterId) {
        ...
        if (result.failed.length > 0) {
          console.error('Image upload failed after retries:', result.failed[0].error);
          addToast('Image could not be saved after retries.', 'error');
        } else {
          setUploadedCharacterImages((previous) =>
            previous.map((image) =>
              image.emotionKey === newImage.emotionKey ? { ...image, file: undefined, avatarFile: undefined } : image,
            ),
          );
  ...
  const uploadImagePair = async (characterId: string, image: UploadedCharacterImage): Promise<void> => {
  ...
  const uploadImagePairs = async (characterId: string, images: UploadedCharacterImage[]): Promise<number> => {
    ...
    if (result.failed.length > 0) {
      ...
      setUploadedCharacterImages((previous) =>
        previous.map((image) =>
          succeededEmotionKeys.has(image.emotionKey) ? { ...image, file: undefined, avatarFile: undefined } : image,
        ),
      );
      throw new Error(
  ...
  const uploadPortraits = async (characterId: string, images: UploadedCharacterImage[]): Promise<void> => {
  ...
  const onSubmit = async (data: CharacterCdo | CharacterInfo) => {
    try {
      ...
      setUploadProgress(null);
      const message = getClientErrorMessage(error, 'An error occurred while saving the character.');
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <GlassCard variant="outlined">
                ...
                <FormControl fullWidth sx={{ mt: 1, mb: 2 }}>
                  ...
                  <GlassSelect
                    ...
                    inputProps={{ 'aria-label': getLangText(LANG_KEYS.EMOTION) }}
                  >
          ...
          <Grid size={{ xs: 12, md: 8 }}>
            <Box display="flex" flexDirection="column" gap={2}>
                  ...
                  >
                    <Box>
                      <Typography variant="h6" color="secondary">
                        {getLangText(LANG_KEYS.CHARACTER_LORE)}
                      </Typography>
                    </Box>
                    <GlassButton
                      colorVariant="silver"
                    ...
                    >
                      {getLangText(LANG_KEYS.MANAGE_LORE)}
                    </GlassButton>
                  </Box>
                </GlassCard>
              ) : null}
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <TextField
                          ...
                          helperText={errors.firstMessage?.message || getLangText(LANG_KEYS.FIRST_MESSAGE_HELPER)}
              ...
              <Box display="flex" gap={2} justifyContent="flex-end">
                <GlassButton colorVariant="silver" variant="outlined" onClick={onCancel} disabled={isSubmitting}>
                  {getLangText(LANG_KEYS.CANCEL)}
                </GlassButton>
                <SolidMetallicButton colorVariant="gold" type="submit" disabled={isSubmitting} loading={isSubmitting}>
                  {submitButtonText}
                </SolidMetallicButton>
              </Box>
            </Box>
        ...
        {pendingImageFile && modalImageUrl && (
          <ImageCropModal
            ...
            title={getLangText(cropStage === 'portrait' ? LANG_KEYS.CROP_FULL_PORTRAIT : LANG_KEYS.CROP_EMOTION_AVATAR)}
          />
        )}
      </form>
      {mode === 'edit' && characterInfo ? (
