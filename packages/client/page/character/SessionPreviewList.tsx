...
export const SessionPreviewList: FC<{
  ...
  const { data: sessionRes, isLoading, error } = useSessionApi().getSessionsByUserIdAndCharacterId(userId, characterId);
        ...
        <ListItemText
          primary={
            <Typography variant="body2" color="error">
              {getClientErrorMessage(error, 'Could not load sessions.')}
            </Typography>
          }
        />
      </ListItem>
      ...
      {activeSessions.map((info, index) => (
        ...
        <Fragment key={info.sessionId}>
          <ListItem disablePadding>
            <ListItemButton onClick={() => handleSessionStart(info.sessionId)} sx={{ borderRadius: 1 }}>
              <ListItemText
                disableTypography
                primary={
                  <Box>
