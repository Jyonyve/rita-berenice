...
const CharacterItem: React.FC<{ characterInfo: CharacterInfo; portraitUrls?: PortraitUrlMap }> = ({
  ...
  return (
    <GlassCard sx={characterCardSx} role="button" onClick={handleCharacterPage} contentProps={{ sx: contentSx }}>
...
const NewCharacterItem = () => {
  ...
  return (
    <GlassCard sx={characterCardSx} role="button" onClick={handleNavigate} contentProps={{ sx: { ...contentSx } }}>
      <Box sx={{ width: '100%', display: 'flex', mb: 1 }}>
      ...
      <Grid container spacing={containerSpacing}>
        {characterInfos.map((characterInfo) => (
          <Grid key={characterInfo.characterId} size={{ xs: 6, sm: 4, md: 3, lg: 2 }}>
            <CharacterItem characterInfo={characterInfo} portraitUrls={characterPortraits[characterInfo.characterId]} />
          </Grid>
        ))}
