    ...
    return (
      <List dense disablePadding>
        <ListItem sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          ...
          >
            <GlassCircularProgress colorVariant="silver" aria-label={getLangText(LANG_KEYS.LOADING_STORIES)} />
          </Box>
        </ListItem>
      </List>
    );
          ...
          <ListItemText
            primary={
              <Typography role="alert" variant="body2" color="error">
                {getClientErrorMessage(error, 'Could not load histories.')}
              </Typography>
            }
          />
        </ListItem>
      ...
      {historyRes?.historyInfos.map((info, index) => (
        ...
        <Fragment key={info.historyId}>
          <ListItem disablePadding>
            <ListItemButton onClick={() => handleHistory(info.historyId)} sx={{ borderRadius: 1 }}>
              <ListItemText
                disableTypography
                primary={
                  <Box>
