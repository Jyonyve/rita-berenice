...
router.post(
  ...
  asyncHandler(
    async (req: Request<object, string, ChatTurn>, res: Response<string>): Promise<void> => {
      ...
      console.log(`API HIT: POST ${path} for sessionId: ${req.body.sessionId}, sequence: ${req.body.sequence}`);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    console.log(`API HIT: GET ${path.replace(':sessionId', sessionId)}?beforeSequence=${beforeSequence}`);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response<ChatResponse>): Promise<void> => {
    ...
    console.log(`API HIT: GET ${path.replace(':sessionId', sessionId).replace(':sequence', sequenceParam)}`);
...
router.post(
  ...
  asyncHandler(
    async (
      ...
      const results = await chatService.queryChatTurns(sessionId, queryTexts, where, limit);
